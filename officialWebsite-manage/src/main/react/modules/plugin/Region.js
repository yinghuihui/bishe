...
var Region = React.createClass({
  ...
  componentWillReceiveProps(nextProps) {
    ...
    }else if(typeof selectedArea == 'undefined'){
      this.setState({
          selectedProvince:'',
          selectedCity:'',
          selectedArea:'',
  ...
  componentDidMount() {
        ...
        this._selectedProvinceName = this._data[this.state.selectedProvince]?this._data[this.state.selectedProvince][0]:'';

        //过滤省对于的市
        var city = this._filter(this.state.selectedProvince)?this._filter(this.state.selectedProvince):'';

        //市的名字
        this._selectedCityName = '';
        if (this.state.selectedCity) {
          this._selectedCityName = this._data[this.state.selectedCity]?this._data[this.state.selectedCity][0]:'';
        }

        //过滤第一个市对应的区
        var area = [];
        if (this.state.selectedCity) {
          area = this._filter(this.state.selectedCity)?this._filter(this.state.selectedCity):'';

          this._selectAreaName = '';
          if (this.state.selectedArea) {
            this._selectAreaName = this._data[this.state.selectedArea]?this._data[this.state.selectedArea][0]:'';
          }
        }
  ...
  render() {
    ...
    return (
      <div>     
          <Select size="large" style={{width:"100px",marginRight: "3px"}} disabled={citydisabe} name="Level0id"
        ...
        <Select size="large" style={{width:"100px",marginRight: "3px"}} disabled={citydisabe} name="Level1id"
         value={state.selectedCity} onChange={this._handleCityChange }>
           {this.state.city.map((v, k) => {
                return <Option key={k} value={String(v[0])}>{v[1]}</Option> 
              })}
        </Select>   
        <Select size="large" style={{width:"100px",marginRight: "3px"}} disabled={citydisabe} name="areaId"
