package com.rongdu.ow.core.common.shiro.realm;
...
private static final Logger logger = Logger.getLogger(ShiroRealm.class);
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {  
       ...
       List<SysMenu> perms = sysMenuService.permByUserName(userName);
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
		...
		try {
			...
			if (user == null) {
				...
				password = PasswordHelper.encryptPassword(user.getSalt(), password);
				if(!(user.getPassword().equals(password))){
					throw new IncorrectCredentialsException(); //密码错误
				}else{
					Subject subject = SecurityUtils.getSubject();
					Session session = subject.getSession();
					session.setAttribute(Constant.SESSION_SYSUSER, user);
