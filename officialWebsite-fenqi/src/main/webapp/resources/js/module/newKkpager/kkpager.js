/**
 * 根据kkpager分页插件来的[基于kkpager的二次开发]
 * 支持一个页面多个分页
 * demo 演示地址 “http://runjs.cn/code/amnrgbc1”
 */
;
(function($, window, undefind) {
    ...
    KingPaging.prototype = {
        ...
        paintPagingHtml: function(config, enforceInit) { //生成控件代码
            ...
            if (this.options.isShowFirstPageBtn) {
                if (this.options.hasPrv) {
                    str_first = '<a ' + this._getHandlerStr(1) + ' title="' + (this.options.lang.firstPageTipText || this.options.lang.firstPageText) + '">' + this.options.lang.firstPageText + '</a>';
                } else {
            ...
            if (this.options.isShowPrePageBtn) {
                if (this.options.hasPrv) {
                    str_prv = '<a ' + this._getHandlerStr(this.options.prv) + ' title="' + (this.options.lang.prePageTipText || this.options.lang.prePageText) + '">' + this.options.lang.prePageText + '</a>';
                } else {
            ...
            if (this.options.isShowNextPageBtn) {
                if (this.options.hasNext) {
                    str_next = '<a ' + this._getHandlerStr(this.options.next) + ' title="' + (this.options.lang.nextPageTipText || this.options.lang.nextPageText) + '">' + this.options.lang.nextPageText + '</a>';
                } else {
            ...
            if (this.options.isShowLastPageBtn) {
                if (this.options.hasNext) {
                    str_last = '<a ' + this._getHandlerStr(this.options.total) + ' title="' + (this.options.lang.lastPageTipText || this.options.lang.lastPageText) + '">' + this.options.lang.lastPageText + '</a>';
                } else {
            ...
            if (this.options.total <= 8) {
                for (var i = 1; i <= this.options.total; i++) {
                    if (this.options.pno == i) {
                        ...
                        str_paging += '<a ' + this._getHandlerStr(i) + ' title="' + this.options.lang.buttonTipBeforeText + i + this.options.lang.buttonTipAfterText + '">' + i + '</a>';
                    }
                }
            } else {
                if (this.options.pno <= 5) {
                    ...
                    str_paging += '<a ' + this._getHandlerStr(1) + ' title="' + this.options.lang.buttonTipBeforeText + '1' + this.options.lang.buttonTipAfterText + '">1</a>';
                    ...
                    if (end != this.options.total) {
                        str_paging += dot;
                        //add
                        str_paging += '<a ' + this._getHandlerStr(this.options.total) + ' title="' + this.options.lang.buttonTipBeforeText + this.options.total + this.options.lang.buttonTipAfterText + '">' + this.options.total + '</a>';
                    }
                }
            }
            //显示总页/数据条数
            ...
            if (this.options.isGoPage) {
                str_gopage_info = '<div class="gopageDivWrap">' + this.options.lang.gopageBeforeText + '<span class="' + this.options.gopageWrapClass + '">' +
                    '<input type="text" class="' + this.options.gopageTextClass + '" value="' + this.options.next + '" /></span>' + this.options.lang.gopageAfterText + "</div>";
            }
        ...
        focus_gopage: function() {
            ...
            $wrapGo.css('border-color', '#e43f47');
            $btnGo.css({
                'left': '0px'
            }).show().animate({
                left: '+=45'
