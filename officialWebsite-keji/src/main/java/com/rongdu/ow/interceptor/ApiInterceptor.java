...
import com.rongdu.ow.user.bean.AppDbSession;
import com.rongdu.ow.user.bean.AppSessionBean;
...
//		}//版本控制结束
		...
		if (StringUtil.isNotBlank(token) && uri.contains("/act/")) {
			...
			if (result instanceof AppSessionBean) {
				...
				logger.info("userData:"+sessionBean.getSession()+"userId:"+sessionBean.getUserId());
